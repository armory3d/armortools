...
let inpaint_image_node_result: gpu_texture_t = null;

function inpaint_image_node_init() {
	array_push(nodes_material_neural, inpaint_image_node_def);
	map_set(parser_material_node_vectors, "NEURAL_INPAINT_IMAGE", inpaint_image_node_vector);
	map_set(ui_nodes_custom_buttons, "inpaint_image_node_button", inpaint_image_node_button);
}

function inpaint_image_node_vector(node: ui_node_t, socket: ui_node_socket_t): string {
	if (inpaint_image_node_result == null) {
		return "float3(0.0, 0.0, 0.0)";
	}
	let tex_name: string = parser_material_node_name(node);
	map_set(data_cached_images, tex_name, inpaint_image_node_result);
	...
	if (iron_exec_async_done == 0) {
		...
		if (input != null) {
			...
			sys_notify_on_next_frame(function(input: gpu_texture_t) {
				...
				sys_notify_on_update(inpaint_image_node_check_result, dir);
			}, input);
		}
	}
}

function inpaint_image_node_check_result(dir: string) {
	if (iron_exec_async_done == 1) {
		let file: string = dir + path_sep + "output.png";
		if (iron_file_exists(file)) {
			inpaint_image_node_result = iron_load_texture(file);
		}
		sys_remove_update(inpaint_image_node_check_result);
	}
}
