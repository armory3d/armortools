...
let upscale_image_node_result: gpu_texture_t = null;

function upscale_image_node_init() {
	array_push(nodes_material_neural, upscale_image_node_def);
	map_set(parser_material_node_vectors, "NEURAL_UPSCALE_IMAGE", upscale_image_node_vector);
	map_set(ui_nodes_custom_buttons, "upscale_image_node_button", upscale_image_node_button);
}

function upscale_image_node_vector(node: ui_node_t, socket: ui_node_socket_t): string {
	if (upscale_image_node_result == null) {
		return "float3(0.0, 0.0, 0.0)";
	}
	let tex_name: string = parser_material_node_name(node);
	map_set(data_cached_images, tex_name, upscale_image_node_result);
...
function upscale_image_node_button(node_id: i32) {
	...
	if (iron_exec_async_done == 0) {
		...
		if (input != null) {
			...
			sys_notify_on_update(upscale_image_node_check_result, dir);
		}
	}
}

function upscale_image_node_check_result(dir: string) {
	if (iron_exec_async_done == 1) {
		let file: string = dir + path_sep + "output.png";
		if (iron_file_exists(file)) {
			upscale_image_node_result = iron_load_texture(file);
		}
		sys_remove_update(upscale_image_node_check_result);
	}
}
