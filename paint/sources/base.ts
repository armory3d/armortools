...
function base_render() {
	...
	if (base_is_dragging) {
		...
		base_drag_rect == null ? draw_scaled_image(img, mouse_x + base_drag_off_x, mouse_y + base_drag_off_y + inv, size, h - inv * 2)
		                       : draw_scaled_sub_image(img, base_drag_rect.x, base_drag_rect.y, base_drag_rect.w, base_drag_rect.h, mouse_x + base_drag_off_x,
		                                               mouse_y + base_drag_off_y + inv, size, h - inv * 2);
...
function ui_base_init_hwnd_tabs(): tab_draw_array_t[] {
	let a0: tab_draw_array_t = [ _draw_callback_create(tab_layers_draw), _draw_callback_create(tab_history_draw), _draw_callback_create(tab_plugins_draw) ];
	let a1: tab_draw_array_t = [
		_draw_callback_create(tab_materials_draw), _draw_callback_create(tab_brushes_draw), _draw_callback_create(tab_scripts_draw)

	];
	let a2: tab_draw_array_t = [
		_draw_callback_create(tab_browser_draw), _draw_callback_create(tab_meshes_draw), _draw_callback_create(tab_textures_draw),
		_draw_callback_create(tab_fonts_draw), _draw_callback_create(tab_swatches_draw), _draw_callback_create(tab_console_draw),
...
function ui_base_update() {
			...
			if (context_raw.brush_locked) {
				if (operator_shortcut(map_get(config_keymap, "brush_opacity"), shortcut_type_t.DOWN)) {
					...
					make_material_parse_paint_material();
				         ...
				         operator_shortcut(map_get(config_keymap, "decal_mask") + "+" + map_get(config_keymap, "brush_radius"), shortcut_type_t.DOWN)) {
	...
	if (context_in_paint_area() && !is_typing) {
		...
		// Radius
		if (context_raw.tool == tool_type_t.BRUSH || context_raw.tool == tool_type_t.ERASER || context_raw.tool == tool_type_t.DECAL ||
		    context_raw.tool == tool_type_t.TEXT || context_raw.tool == tool_type_t.CLONE || context_raw.tool == tool_type_t.BLUR ||
		    context_raw.tool == tool_type_t.SMUDGE || context_raw.tool == tool_type_t.PARTICLE) {
			if (operator_shortcut(map_get(config_keymap, "brush_radius")) || operator_shortcut(map_get(config_keymap, "brush_opacity")) ||
				...
				else if (operator_shortcut(map_get(config_keymap, "decal_mask") + "+" + map_get(config_keymap, "brush_radius_increase"),
				                           shortcut_type_t.REPEAT)) {
					context_raw.brush_decal_mask_radius += ui_base_get_radius_increment();
					context_raw.brush_decal_mask_radius          = math_round(context_raw.brush_decal_mask_radius * 100) / 100;
					context_raw.brush_decal_mask_radius_handle.f = context_raw.brush_decal_mask_radius;
					ui_header_handle.redraws                     = 2;
		...
		if (decal_mask && (operator_shortcut(map_get(config_keymap, "decal_mask"), shortcut_type_t.STARTED) ||
		                   operator_shortcut(map_get(config_keymap, "decal_mask"), shortcut_type_t.RELEASED))) {
			ui_header_handle.redraws = 2;
		}
		...
		if (mouse_view_x() < sys_w()) {
			if (operator_shortcut(map_get(config_keymap, "view_reset"))) {
				...
				ui_menu_draw(function() {
					...
					let modes: string[] = [
						tr("Lit"), tr("Base Color"), tr("Normal"), tr("Occlusion"), tr("Roughness"), tr("Metallic"), tr("Opacity"), tr("Height"),
						tr("Emission"), tr("Subsurface"), tr("TexCoord"), tr("Object Normal"), tr("Material ID"), tr("Object ID"), tr("Mask")
					];
	...
	if (context_raw.brush_locked) {
		let b: bool =
		    context_raw.brush_locked && !operator_shortcut(map_get(config_keymap, "brush_radius"), shortcut_type_t.DOWN) &&
		    !operator_shortcut(map_get(config_keymap, "brush_opacity"), shortcut_type_t.DOWN) &&
		    !operator_shortcut(map_get(config_keymap, "brush_angle"), shortcut_type_t.DOWN) &&
		    !(decal_mask && operator_shortcut(map_get(config_keymap, "decal_mask") + "+" + map_get(config_keymap, "brush_radius"), shortcut_type_t.DOWN));
	...
	if (ui_base_border_handle != null) {
		if (ui_base_border_handle == ui_nodes_hwnd || ui_base_border_handle == ui_view2d_hwnd) {
			...
			if (config_raw.layout[layout_size_t.STATUS_H] - my >= ui_statusbar_default_h * config_raw.window_scale &&
			    config_raw.layout[layout_size_t.STATUS_H] - my < iron_window_height() * 0.7) {
...
function ui_base_update_ui() {
	...
	if (context_raw.brush_stencil_image != null && operator_shortcut(map_get(config_keymap, "stencil_transform"), shortcut_type_t.DOWN)) {
		...
		if (mouse_started("left")) {
			context_raw.brush_stencil_scaling =
			    ui_base_hit_rect(mouse_x, mouse_y, r.x - 8, r.y - 8, 16, 16) || ui_base_hit_rect(mouse_x, mouse_y, r.x - 8, r.h + r.y - 8, 16, 16) ||
			    ui_base_hit_rect(mouse_x, mouse_y, r.w + r.x - 8, r.y - 8, 16, 16) || ui_base_hit_rect(mouse_x, mouse_y, r.w + r.x - 8, r.h + r.y - 8, 16, 16);
	...
	let redo_pressed: bool = operator_shortcut(map_get(config_keymap, "edit_redo")) || (keyboard_down("control") && keyboard_started("y"));
...
function ui_base_render_cursor() {
	...
	if (context_raw.brush_stencil_image != null && context_raw.tool != tool_type_t.PICKER && context_raw.tool != tool_type_t.COLORID) {
		let r: rect_t = ui_base_get_brush_stencil_rect();
	...
	if (context_in_2d_view() || decal) {
		...
		if (context_raw.tool == tool_type_t.BRUSH || context_raw.tool == tool_type_t.ERASER || context_raw.tool == tool_type_t.CLONE ||
	...
	if (context_raw.brush_lazy_radius > 0 && !context_raw.brush_locked &&
	    (context_raw.tool == tool_type_t.BRUSH || context_raw.tool == tool_type_t.ERASER || context_raw.tool == tool_type_t.DECAL ||
	     context_raw.tool == tool_type_t.TEXT || context_raw.tool == tool_type_t.CLONE || context_raw.tool == tool_type_t.BLUR ||
	     context_raw.tool == tool_type_t.SMUDGE || context_raw.tool == tool_type_t.PARTICLE)) {
...
function ui_base_on_border_hover(handle: ui_handle_t, side: i32) {
	...
	if (handle != ui_base_hwnds[tab_area_t.SIDEBAR0] && handle != ui_base_hwnds[tab_area_t.SIDEBAR1] && handle != ui_base_hwnds[tab_area_t.STATUS] &&
	    handle != ui_nodes_hwnd && handle != ui_view2d_hwnd) {
