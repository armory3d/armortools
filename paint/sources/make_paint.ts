...
function make_paint_run(data: material_t, matcon: material_context_t): node_shader_context_t {
	...
	if (context_raw.brush_stencil_image != null &&
	    (context_raw.tool == tool_type_t.BRUSH || context_raw.tool == tool_type_t.ERASER || context_raw.tool == tool_type_t.FILL ||
	     context_raw.tool == tool_type_t.CLONE || context_raw.tool == tool_type_t.BLUR || context_raw.tool == tool_type_t.SMUDGE ||
	     context_raw.tool == tool_type_t.PARTICLE || decal)) {
	...
	if (context_raw.brush_mask_image != null && (context_raw.tool == tool_type_t.BRUSH || context_raw.tool == tool_type_t.ERASER)) {
		...
		if (context_raw.brush_directional) {
			...
			node_shader_write_frag(
			    kong,
			    "pa_mask = float2(pa_mask.x * constants.brush_direction.x - pa_mask.y * constants.brush_direction.y, pa_mask.x * constants.brush_direction.y + pa_mask.y * constants.brush_direction.x);");
		}
		...
		if (angle != 0.0) {
			...
			node_shader_write_frag(
			    kong,
			    "pa_mask.xy = float2(pa_mask.x * constants.brush_angle.x - pa_mask.y * constants.brush_angle.y, pa_mask.x * constants.brush_angle.y + pa_mask.y * constants.brush_angle.x);");
		}
