...
viewport_mode_t ui_menubar_string_to_viewport_mode(char *s) {
	if (string_equals(s, tr("Lit", NULL)))
		return VIEWPORT_MODE_LIT;
	if (string_equals(s, tr("Base Color", NULL)))
		return VIEWPORT_MODE_BASE_COLOR;
	if (string_equals(s, tr("Normal", NULL)))
		return VIEWPORT_MODE_NORMAL_MAP;
	if (string_equals(s, tr("Occlusion", NULL)))
		return VIEWPORT_MODE_OCCLUSION;
	if (string_equals(s, tr("Roughness", NULL)))
		return VIEWPORT_MODE_ROUGHNESS;
	if (string_equals(s, tr("Metallic", NULL)))
		return VIEWPORT_MODE_METALLIC;
	if (string_equals(s, tr("Opacity", NULL)))
		return VIEWPORT_MODE_OPACITY;
	if (string_equals(s, tr("Height", NULL)))
		return VIEWPORT_MODE_HEIGHT;
	if (string_equals(s, tr("Emission", NULL)))
		return VIEWPORT_MODE_EMISSION;
	if (string_equals(s, tr("Subsurface", NULL)))
		return VIEWPORT_MODE_SUBSURFACE;
	if (string_equals(s, tr("TexCoord", NULL)))
		return VIEWPORT_MODE_TEXCOORD;
	if (string_equals(s, tr("Object Normal", NULL)))
		return VIEWPORT_MODE_OBJECT_NORMAL;
	if (string_equals(s, tr("Material ID", NULL)))
		return VIEWPORT_MODE_MATERIAL_ID;
	if (string_equals(s, tr("Object ID", NULL)))
		return VIEWPORT_MODE_OBJECT_ID;
	if (string_equals(s, tr("Mask", NULL)))
		return VIEWPORT_MODE_MASK;
	if (string_equals(s, tr("Path Traced", NULL)))
...
void ui_menubar_draw_category_items() {
	...
	if (ui_menu_button(tr("Undo {step}", vars_undo), any_map_get(config_keymap, "edit_undo"), ICON_UNDO)) {
		history_undo();
	}
	...
	if (ui_menu_button(tr("Redo {step}", vars_redo), any_map_get(config_keymap, "edit_redo"), ICON_REDO)) {
		history_redo();
	}
	ui->enabled = true;
	ui_menu_separator();
...
#if defined(IRON_MACOS) || defined(IRON_IOS) // Limited url length
		iron_load_url(string("https://github.com/armory3d/armortools/issues/new?labels=feature%%20request&template=feature_request.md&body=*%s%%20%s-%s,%%20%s",
		                     manifest_title, manifest_version, config_get_sha(), iron_system_id()));
#else
	...
	ui_menu_separator();
	ui_menu_align();
	ui_menu_label(tr("Workflow", NULL), NULL);
	ui_menu_align();
