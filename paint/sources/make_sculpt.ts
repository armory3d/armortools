...
function sculpt_make_sculpt_run(data: material_t, matcon: material_context_t): node_shader_context_t {
	...
	if (context_raw.tool == tool_type_t.BRUSH || context_raw.tool == tool_type_t.ERASER || context_raw.tool == tool_type_t.CLONE ||
	    context_raw.tool == tool_type_t.BLUR || context_raw.tool == tool_type_t.SMUDGE || context_raw.tool == tool_type_t.PARTICLE || decal) {
...
function sculpt_make_mesh_run(kong: node_shader_t, l: slot_layer_t) {
	...
	node_shader_write_vert(
	    kong,
	    "var meshpos: float4 = texpaint_sculpt[uint2(uint(float(vertex_id()) % constants.texpaint_sculpt_size.x), uint(float(vertex_id()) / constants.texpaint_sculpt_size.y))];");
	...
	node_shader_write_attrib_vert(
	    kong,
	    "var meshpos0: float4 = texpaint_sculpt[uint2(uint(base_vertex0 % constants.texpaint_sculpt_size.x), uint(base_vertex0 / constants.texpaint_sculpt_size.y))];");
	node_shader_write_attrib_vert(
	    kong,
	    "var meshpos1: float4 = texpaint_sculpt[uint2(uint(base_vertex1 % constants.texpaint_sculpt_size.x), uint(base_vertex1 / constants.texpaint_sculpt_size.y))];");
	node_shader_write_attrib_vert(
	    kong,
	    "var meshpos2: float4 = texpaint_sculpt[uint2(uint(base_vertex2 % constants.texpaint_sculpt_size.x), uint(base_vertex2 / constants.texpaint_sculpt_size.y))];");
...
function sculpt_layers_create_sculpt_layer() {
	...
	let raw: mesh_data_t = {
		name : md.name,
		vertex_arrays : [
			{values : posa, attrib : "pos", data : "short4norm"}, {values : nora, attrib : "nor", data : "short2norm"},
			{values : texa, attrib : "tex", data : "short2norm"}
		],
		index_array : inda,
		scale_pos : 1.0,
		scale_tex : 1.0
