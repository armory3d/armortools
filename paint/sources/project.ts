...
function project_cleanup() {
	...
	for (let i: i32 = 0; i < len; ++i) {
		...
		if (array_index_of(context_raw.project_objects, m) == -1 && m.base.name != ".ParticleEmitter" && m.base.name != ".Particle") {
...
function project_new(reset_layers: bool = true) {
	...
	if (reset_layers) {
		let aspect_ratio_changed: bool =
		    project_layers[0].texpaint.width != config_get_texture_res_x() || project_layers[0].texpaint.height != config_get_texture_res_y();
...
function project_create_node_link(links: ui_node_link_t[], from_id: i32, from_socket: i32, to_id: i32, to_socket: i32): ui_node_link_t {
	let link: ui_node_link_t = {id : ui_next_link_id(links), from_id : from_id, from_socket : from_socket, to_id : to_id, to_socket : to_socket};
	return link;
}
...
function project_get_image(asset: asset_t): gpu_texture_t {
...
function make_swatch(base: i32 = 0xffffffff): swatch_color_t {
	let s: swatch_color_t =
	    {base : base, opacity : 1.0, occlusion : 1.0, roughness : 0.0, metallic : 0.0, normal : 0xff8080ff, emission : 0.0, height : 0.0, subsurface : 0.0};
	return s;
}
