...
function camera_update() {
	...
	if (operator_shortcut(map_get(config_keymap, "action_rotate"), shortcut_type_t.STARTED) ||
	    ...
	    operator_shortcut(map_get(config_keymap, "rotate_envmap"), shortcut_type_t.STARTED) || (mouse_started("right") && !modif) ||
		...
		camera_controls_down = true;
	         ...
	         !operator_shortcut(map_get(config_keymap, "rotate_envmap"), shortcut_type_t.DOWN) && !(mouse_down("right") && !modif) &&
	...
	if (controls == camera_controls_t.ORBIT &&
	    (operator_shortcut(map_get(config_keymap, "action_rotate"), shortcut_type_t.DOWN) || (mouse_down("right") && !modif && default_keymap))) {
		camera_redraws = 2;
	...
	else if (controls == camera_controls_t.ROTATE &&
	         (operator_shortcut(map_get(config_keymap, "action_rotate"), shortcut_type_t.DOWN) || (mouse_down("right") && !modif && default_keymap))) {
		camera_redraws     = 2;
...
function camera_get_zoom_speed(): f32 {
	let sign: i32 = config_raw.zoom_direction == zoom_direction_t.VERTICAL_INVERTED || config_raw.zoom_direction == zoom_direction_t.HORIZONTAL_INVERTED ||
	                        config_raw.zoom_direction == zoom_direction_t.VERTICAL_HORIZONTAL_INVERTED
	                    ? -1
	                    : 1;
	let camera: camera_object_t = scene_camera;
