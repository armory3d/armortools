...
function uniforms_ext_tex_link(object: object_t, mat: material_data_t, link: string): gpu_texture_t {
	if (link == "_texpaint_undo") {
		...
		if (!util_uv_uvmap_cached) {
			sys_notify_on_next_frame(function(_: any) { util_uv_cache_uv_map(); });
		}
		return util_uv_uvmap;
	}
	else if (link == "_textrianglemap") {
		if (!util_uv_trianglemap_cached) {
			sys_notify_on_next_frame(function(_: any) { util_uv_cache_triangle_map(); });
		}
		return util_uv_trianglemap;
	}
	else if (link == "_texuvislandmap") {
		sys_notify_on_next_frame(function(_: any) { util_uv_cache_uv_island_map(); });
