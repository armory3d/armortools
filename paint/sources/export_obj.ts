	...
	for (let i: i32 = 0; i < paint_objects.length; ++i) {
		...
		for (let i: i32 = 0; i < len; ++i) {
			...
			for (let j: i32 = 0; j < pi; ++j) {
				if (posa2[j * 3] == posa[i * 4] && posa2[j * 3 + 1] == posa[i * 4 + 1] && posa2[j * 3 + 2] == posa[i * 4 + 2]) {
					posmap[i] = j;
					found     = true;
					break;
				}
			...
			for (let j: i32 = 0; j < ni; ++j) {
				if (nora2[j * 3] == nora[i * 2] && nora2[j * 3 + 1] == nora[i * 2 + 1] && nora2[j * 3 + 2] == posa[i * 4 + 3]) {
					normap[i] = j;
					found     = true;
					break;
				}
			...
			for (let j: i32 = 0; j < ti; ++j) {
				if (texa2[j * 2] == texa[i * 2] && texa2[j * 2 + 1] == texa[i * 2 + 1]) {
					texmap[i] = j;
					found     = true;
					break;
				}
