...
function ui_sidebar_render_ui() {
	...
	if (config_raw.layout[layout_size_t.SIDEBAR_W] == 0) {
		...
		if (ui_window(ui_sidebar_hminimized, iron_window_width() - width, -1, width, math_floor(UI_ELEMENT_H() + 4 * UI_SCALE()))) {
			...
			if (ui_button("<")) {
				config_raw.layout[layout_size_t.SIDEBAR_W] =
	...
	if (ui_window(ui_base_hwnds[tab_area_t.SIDEBAR1], ui_sidebar_tabx, config_raw.layout[layout_size_t.SIDEBAR_H0], config_raw.layout[layout_size_t.SIDEBAR_W],
	              config_raw.layout[layout_size_t.SIDEBAR_H1] - expand_button_offset)) {
