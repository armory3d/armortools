...
kinc_g5_constant_location_t kinc_g4_pipeline_get_constant_location(kinc_g4_pipeline_t *pipe, const char *name) {
	kinc_g5_constant_location_t location;
	location = kinc_g5_pipeline_get_constant_location(&pipe->impl._pipeline, name);
	return location;
}

kinc_g5_texture_unit_t kinc_g4_pipeline_get_texture_unit(kinc_g4_pipeline_t *pipe, const char *name) {
	kinc_g5_texture_unit_t g5_unit = kinc_g5_pipeline_get_texture_unit(&pipe->impl._pipeline, name);

	kinc_g5_texture_unit_t g4_unit;
	memcpy(&g4_unit.stages[0], &g5_unit.stages[0], KINC_G5_SHADER_TYPE_COUNT * sizeof(int));

	return g4_unit;
}

void kinc_g4_pipeline_compile(kinc_g4_pipeline_t *pipe) {
	pipe->impl._pipeline.input_layout = pipe->input_layout;
	pipe->impl._pipeline.vertex_shader = pipe->vertex_shader;
	pipe->impl._pipeline.fragment_shader = pipe->fragment_shader;
	...
	pipe->impl._pipeline.cull_mode = (kinc_g5_cull_mode_t)pipe->cull_mode;
...
void kinc_g4_shader_init(kinc_g5_shader_t *shader, const void *_data, size_t length, kinc_g5_shader_type_t type) {
	kinc_g5_shader_init(shader, _data, length, type);
}
...
int kinc_g4_shader_init_from_source(kinc_g5_shader_t *shader, const char *source, kinc_g5_shader_type_t type) {
	...
	int errors = krafix_compile(source, output, &length, target, system, type == KINC_G5_SHADER_TYPE_FRAGMENT ? "frag" : "vert", -1);
	if (errors > 0) {
		return errors;
	}
