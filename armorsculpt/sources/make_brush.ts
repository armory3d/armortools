...
function make_brush_run(kong: node_shader_t) {
	...
	if (config_raw.brush_3d) {
		...
		node_shader_write_attrib_frag(kong, "var wposition: float3 = (W * texpaint_undo[int2(tex_coord.x * textureSize(texpaint_undo, 0).x, tex_coord.y * textureSize(texpaint_undo, 0).y)]).xyz;");
