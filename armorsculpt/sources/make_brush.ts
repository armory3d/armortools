...
function make_brush_run(kong: node_shader_t) {
	...
	if (config_raw.brush_3d) {
		node_shader_write_frag(kong, "var depth: float = sample_lod(gbufferD, sampler_linear, inp.xy, 0.0).r;");
		...
		node_shader_write_attrib_frag(kong, "var wposition: float3 = (constants.W * texpaint_undo[uint2(input.tex_coord.x * constants.texpaint_undo_size.x, input.tex_coord.y * constants.texpaint_undo_size.y)]).xyz;");
