...
value_t eval_object(interpreter_t *interp) {
	...
	while (interp->tokens[interp->pos].type != TOK_RBRACE) {
		...
		obj.data.obj.fields                                = realloc(obj.data.obj.fields, sizeof(value_t) * (obj.data.obj.field_count + 1));
		obj.data.obj.field_names                           = realloc(obj.data.obj.field_names, sizeof(char *) * (obj.data.obj.field_count + 1));
...
value_t eval_expression(interpreter_t *interp) {
	...
	if (interp->tokens[interp->pos].type == TOK_NUM || interp->tokens[interp->pos].type == TOK_STR) {
	...
	while (interp->pos < interp->token_count) {
		...
		if (op != TOK_PLUS && op != TOK_MINUS && op != TOK_MUL && op != TOK_DIV && op != TOK_LT && op != TOK_GT && op != TOK_LE && op != TOK_GE &&
		    op != TOK_EQEQ && op != TOK_NE)
			break;
		interp->pos++;
		...
		if (left.type == VAL_NUM && right.type == VAL_NUM) {
			...
			case TOK_LT:
				left.type     = VAL_NUM;
				left.data.num = left.data.num < right.data.num;
				break;
			case TOK_GT:
				left.type     = VAL_NUM;
				left.data.num = left.data.num > right.data.num;
...
void execute_statement(interpreter_t *interp) {
	while (interp->pos < interp->token_count && interp->tokens[interp->pos].type != TOK_RBRACE) {
