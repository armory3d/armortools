...
class ImportEnvmap {
	...
	static run = (path: string, image: Image) => {
		...
		// Irradiance
		Scene.active.world.probe.irradiance = ImportEnvmap.getSphericalHarmonics(radiancePixels, ImportEnvmap.radiance.width, ImportEnvmap.radiance.height);

		// World
		Scene.active.world.probe.raw.strength = 1.0;
		Scene.active.world.probe.raw.radiance_mipmaps = ImportEnvmap.mipsCpu.length - 2;
		Scene.active.world.envmap = image;
		Scene.active.world.raw.envmap = path;
		Scene.active.world.probe.radiance = ImportEnvmap.radianceCpu;
		Scene.active.world.probe.radianceMipmaps = ImportEnvmap.mipsCpu;
