...
class UniformsExt {
	...
	public static function linkFloat(object: Object, mat: MaterialData, link: String): Null<Float> {
		...
		if (ParserMaterial.script_links != null) {
			for (key in ParserMaterial.script_links.keys()) {
				var script = ParserMaterial.script_links[key];
				var result = 0.0;
				if (script != "") {
					try {
	...
	public static function linkTex(object: Object, mat: MaterialData, link: String): Image {
		...
		if (link.startsWith("_texpaint_vert")) {
			...
			#if is_lab
			return arm.nodes.BrushOutputNode.inst.texpaint;
			#end
		}
		if (link.startsWith("_texpaint_nor")) {
			...
			#if is_lab
			return arm.nodes.BrushOutputNode.inst.texpaint_nor;
			#end
		}
		if (link.startsWith("_texpaint_pack")) {
			...
			#if is_lab
			return arm.nodes.BrushOutputNode.inst.texpaint_pack;
			#end
		}
