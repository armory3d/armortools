	...
	public function drawToolProperties(ui: Zui) {
		if (Context.raw.tool == ToolColorId) {
			...
			if (!baking && ui.button(tr("Bake"))) {
				...
				#if (krom_direct3d12 || krom_vulkan || krom_metal)
				arm.RenderPathRaytraceBake.currentSample = 0;
				#end
			}
			...
			#if (krom_direct3d12 || krom_vulkan || krom_metal)
			if (rtBake) {
				var progress = arm.RenderPathRaytraceBake.currentSample / Context.raw.bakeSamples;
				...
				ui.text(tr("Samples") + ": " + arm.RenderPathRaytraceBake.currentSample);
				ui.text(tr("Rays/pixel" + ": ") + arm.RenderPathRaytraceBake.raysPix);
				ui.text(tr("Rays/second" + ": ") + arm.RenderPathRaytraceBake.raysSec);
			}
			#end
			...
			if (Context.raw.tool == ToolParticle) {
				...
				if (physHandle.changed) {
					arm.UtilParticle.initParticlePhysics();
					MakeMaterial.parsePaintMaterial();
				}
			}
			#end
