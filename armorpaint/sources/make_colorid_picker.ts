...
function make_colorid_picker_run(kong: node_shader_t) {
	...
	if (context_raw.tool == workspace_tool_t.COLORID) {
		...
		if (context_raw.pick_pos_nor_tex) {
			...
			node_shader_write_frag(kong, "output[0].rgba = float4(get_pos_from_depth(float2(constants.inp.x, 1.0 - constants.inp.y), constants.invVP, texturePass(gbufferD)), tex_coord_inp.x);");
			node_shader_write_frag(kong, "output[1].rgba = float4(get_nor_from_depth(output[0].rgb, float2(constants.inp.x, 1.0 - constants.inp.y), constants.invVP, float2(1.0, 1.0) / constants.gbuffer_size, texturePass(gbufferD)), tex_coord_inp.y);");
		}
		else {
