...
function make_bake_run(con: node_shader_context_t, kong: node_shader_t) {
	if (context_raw.bake_type == bake_type_t.CURVATURE) {
		...
		node_shader_write_frag(kong, "var n0: float3 = sample_lod(texpaint_undo, sampler_linear, input.tex_coord, 0.0).rgb * float3(2.0, 2.0, 2.0) - float3(1.0, 1.0, 1.0);");
		...
		node_shader_write_frag(kong, "var wpos0: float3 = sample_lod(texpaint_undo, sampler_linear, input.tex_coord, 0.0).rgb * float3(2.0, 2.0, 2.0) - float3(1.0, 1.0, 1.0);");
		...
		node_shader_write_frag(kong, "var h0: float = sample_lod(texpaint_undo, sampler_linear, input.tex_coord, 0.0).r * 100;");
		node_shader_write_frag(kong, "var h1: float = sample_lod(texpaint_undo, sampler_linear, input.tex_coord + tex_dx, 0.0).r * 100;");
		node_shader_write_frag(kong, "var h2: float = sample_lod(texpaint_undo, sampler_linear, input.tex_coord + tex_dy, 0.0).r * 100;");
		...
		node_shader_write_frag(kong, "output[0] = float4(input.tex_coord.xy, 0.0, 1.0);");
	}
	else if (context_raw.bake_type == bake_type_t.MATERIALID) {
		node_shader_add_texture(kong, "texpaint_nor_undo", "_texpaint_nor_undo");
		node_shader_write_frag(kong, "var sample_matid: float = sample_lod(texpaint_nor_undo, sampler_linear, input.tex_coord, 0.0).a + 1.0 / 255.0;");
