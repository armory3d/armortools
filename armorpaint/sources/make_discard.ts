...
function make_discard_color_id(kong: node_shader_t) {
	...
	node_shader_write_frag(kong, "var colorid_c2: float3 = sample_lod(texcolorid, sampler_linear, tex_coord_pick, 0).rgb;");
...
function make_discard_face(kong: node_shader_t) {
	...
	node_shader_write_frag(kong, "var face_c1: float4 = textrianglemap[uint2(tex_coord_inp * constants.textrianglemap_size)];");
	node_shader_write_frag(kong, "var face_c2: float4 = sample_lod(textrianglemap, sampler_linear, tex_coord_pick, 0.0);");
...
function make_discard_uv_island(kong: node_shader_t) {
	...
	node_shader_write_frag(kong, "if (sample_lod(texuvislandmap, sampler_linear, tex_coord_pick, 0.0).r == 0.0) { discard; }");
}
