...
function make_mesh_run(data: material_t, layer_pass: i32 = 0): node_shader_context_t {
	...
	node_shader_add_uniform(kong, "VP: float4x4", "_view_proj_matrix");
	...
	if (make_material_height_used && displace_strength > 0.0) {
		...
		for (let i: i32 = 0; i < project_layers.length; ++i) {
			...
			num_layers++;
			texture_count++;
			node_shader_add_uniform(kong, "texpaint_pack_vert" + l.id + ": tex2d", "_texpaint_pack_vert" + l.id);
			node_shader_write_vert(kong, "height += sample_lod(texpaint_pack_vert" + l.id + ", tex, 0.0).a;");
			...
			if (masks != null) {
				for (let i: i32 = 0; i < masks.length; ++i) {
					...
					texture_count++;
					node_shader_add_uniform(kong, "texpaint_vert" + m.id + ": tex2d", "_texpaint_vert" + m.id);
					node_shader_write_vert(kong, "height *= sample_lod(texpaint_vert" + m.id + ", tex, 0.0).r;");
				}
			}
		}
	...
	node_shader_write_vert(kong, "output.pos = VP * float4(wposition.xyz, 1.0);");
	node_shader_write_vert(kong, "tex_coord = tex;");
	...
	if (context_raw.tool == workspace_tool_t.COLORID) {
		...
		node_shader_write_frag(kong, "output[0].rgba = float4(n.xy, 1.0, pack_f32_i16(0.0, uint(0)));");
		node_shader_write_frag(kong, "var idcol: float3 = pow(sample_lod(texcolorid, tex_coord, 0.0).rgb, float3(2.2, 2.2, 2.2));");
		node_shader_write_frag(kong, "output[1].rgba = float4(idcol.rgb, 1.0);"); // occ
	}
	else {
		...
		if (layer_pass > 0) {
			...
			node_shader_add_uniform(kong, "gbuffer1: tex2d");
		...
		if (context_raw.draw_wireframe) {
			texture_count++;
			node_shader_add_uniform(kong, "texuvmap: tex2d", "_texuvmap");
		}
		...
		if (context_raw.viewport_mode == viewport_mode_t.MASK && slot_layer_get_masks(context_raw.layer) != null) {
			for (let i: i32 = 0; i < slot_layer_get_masks(context_raw.layer).length; ++i) {
				...
				node_shader_add_uniform(kong, "texpaint_view_mask: tex2d" + m.id, "_texpaint" + index);
			}
		}

		if (context_raw.viewport_mode == viewport_mode_t.LIT && context_raw.render_mode == render_mode_t.FORWARD) {
			texture_count += 4;
			node_shader_add_uniform(kong, "senvmap_brdf: tex2d", "$brdf.k");
			node_shader_add_uniform(kong, "senvmap_radiance: tex2d", "_envmap_radiance");
		}
		...
		for (let i: i32 = 0; i < layers.length; ++i) {
			...
			if (slot_layer_get_object_mask(l) > 0) {
				node_shader_add_uniform(kong, "uid: int", "_uid");
			...
			node_shader_add_shared_sampler(kong, "texpaint" + l.id + ": tex2d");
			node_shader_write_frag(kong, "texpaint_sample = sample_lod_shared(texpaint" + l.id + ", tex_coord, 0.0);");
			...
			if (masks != null) {
				...
				if (has_visible) {
					...
					for (let i: i32 = 0; i < masks.length; ++i) {
						...
						node_shader_add_shared_sampler(kong, "texpaint" + m.id + ": tex2d");
						node_shader_write_frag(kong, "{"); // Group mask is sampled across multiple layers
						node_shader_write_frag(kong, "var texpaint_mask_sample" + m.id + ": float = sample_lod_shared(texpaint" + m.id + ", tex_coord, 0.0).r;");
			...
			if (l.paint_nor || make_material_emis_used) {
				node_shader_add_shared_sampler(kong, "texpaint_nor" + l.id + ": tex2d");
				node_shader_write_frag(kong, "texpaint_nor_sample = sample_lod_shared(texpaint_nor" + l.id + ", tex_coord, 0.0);");
				...
				if (l.paint_nor) {
					if (l.paint_nor_blend) {
						...
						node_shader_write_frag(kong, "var n2: float3 = clamp(float3(0.5, 0.5, 1.0), texpaint_nor_sample.rgb, texpaint_opac) * float3(2.0, 2.0, 2.0) - float3(1.0, 1.0, 1.0);");
						node_shader_write_frag(kong, "ntex = normalize(float3(n1.xy + n2.xy, n1.z * n2.z)) * float3(0.5, 0.5, 0.5) + float3(0.5, 0.5, 0.5);");
						node_shader_write_frag(kong, "}");
					}
					else {
						node_shader_write_frag(kong, "ntex = clamp(ntex, texpaint_nor_sample.rgb, texpaint_opac);");
					}
				}
			}

			if (l.paint_occ || l.paint_rough || l.paint_met || (l.paint_height && make_material_height_used)) {
				node_shader_add_shared_sampler(kong, "texpaint_pack" + l.id + ": tex2d");
				node_shader_write_frag(kong, "texpaint_pack_sample = sample_lod_shared(texpaint_pack" + l.id + ", tex_coord, 0.0);");
				...
				if (l.paint_height && make_material_height_used) {
					...
					node_shader_write_frag(kong, "{");
					node_shader_add_uniform(kong, "texpaint_size: float2", "_texpaint_size");
					node_shader_write_frag(kong, "var tex_step: float = 1.0 / texpaint_size.x;");
					node_shader_write_frag(kong, "height0 " + assign + " sample_lod_shared(texpaint_pack" + l.id + ", float2(tex_coord.x - tex_step, tex_coord.y), 0.0).a * texpaint_opac;");
					node_shader_write_frag(kong, "height1 " + assign + " sample_lod_shared(texpaint_pack" + l.id + ", float2(tex_coord.x + tex_step, tex_coord.y), 0.0).a * texpaint_opac;");
					node_shader_write_frag(kong, "height2 " + assign + " sample_lod_shared(texpaint_pack" + l.id + ", float2(tex_coord.x, tex_coord.y - tex_step), 0.0).a * texpaint_opac;");
					node_shader_write_frag(kong, "height3 " + assign + " sample_lod_shared(texpaint_pack" + l.id + ", float2(tex_coord.x, tex_coord.y + tex_step), 0.0).a * texpaint_opac;");
					node_shader_write_frag(kong, "}");
				}
			}
		...
		if (last_pass && context_raw.draw_texels) {
			node_shader_add_uniform(kong, "texpaint_size: float2", "_texpaint_size");
			node_shader_write_frag(kong, "var texel0: float2 = tex_coord * texpaint_size * 0.01;");
			node_shader_write_frag(kong, "var texel1: float2 = tex_coord * texpaint_size * 0.1;");
			node_shader_write_frag(kong, "var texel2: float2 = tex_coord * texpaint_size;");
		...
		if (last_pass && context_raw.draw_wireframe) {
			node_shader_write_frag(kong, "var wireframe: float = sample_lod(texuvmap, tex_coord, 0.0).a;");
		...
		if (!last_pass) {
			node_shader_write_frag(kong, "output[0].rgba = float4(basecol, opacity);");
			node_shader_write_frag(kong, "output[1].rgba = float4(ntex, matid);");
		...
		if (context_raw.viewport_mode == viewport_mode_t.LIT || context_raw.viewport_mode == viewport_mode_t.PATH_TRACE) {
			...
			if (context_raw.viewport_shader != null) {
				...
				node_shader_write_frag(kong, "var albedo: float3 = clamp(basecol, float3(0.0, 0.0, 0.0), metallic);");
				node_shader_write_frag(kong, "var f0: float3 = clamp(float3(0.04, 0.04, 0.04), basecol, metallic);");
				kong.frag_vvec = true;
				...
				node_shader_add_uniform(kong, "envmap_num_mipmaps: int", "_envmap_num_mipmaps");
				node_shader_add_uniform(kong, "envmap_data: float4", "_envmap_data"); // angle, sin(angle), cos(angle), strength
				node_shader_write_frag(kong, "var wreflect: float3 = reflect(-vvec, n);");
				node_shader_write_frag(kong, "var envlod: float = roughness * float(envmap_num_mipmaps);");
				node_shader_add_function(kong, str_envmap_equirect);
				node_shader_write_frag(kong, "var prefiltered_color: float3 = sample_lod(senvmap_radiance, envmap_equirect(wreflect, envmap_data.x), envlod).rgb;");
				node_shader_add_uniform(kong, "shirr: float4[7]", "_envmap_irradiance");
				node_shader_add_function(kong, str_sh_irradiance);
				node_shader_write_frag(kong, "var indirect: float3 = albedo * (sh_irradiance(float3(n.x * envmap_data.z - n.y * envmap_data.y, n.x * envmap_data.y + n.y * envmap_data.z, n.z), shirr) / 3.14159265);");
			...
			node_shader_write_frag(kong, "float emis = int(matid * 255.0) % 3 == 1 ? 1.0 : 0.0;");
			node_shader_write_frag(kong, "output[1].rgba = float4(float3(emis, emis, emis), 1.0);");
		}
		else if (context_raw.viewport_mode == viewport_mode_t.SUBSURFACE) {
			node_shader_write_frag(kong, "float subs = int(matid * 255.0) % 3 == 2 ? 1.0 : 0.0;");
			node_shader_write_frag(kong, "output[1].rgba = float4(float3(subs, subs, subs), 1.0);");
		}
		else if (context_raw.viewport_mode == viewport_mode_t.TEXCOORD) {
			node_shader_write_frag(kong, "output[1].rgba = float4(tex_coord, 0.0, 1.0);");
		}
		else if (context_raw.viewport_mode == viewport_mode_t.OBJECT_NORMAL) {
			kong.frag_nattr = true;
			node_shader_write_frag(kong, "output[1].rgba = float4(nattr, 1.0);");
		}
		else if (context_raw.viewport_mode == viewport_mode_t.MATERIAL_ID) {
			let id: i32 = context_raw.layer.id;
			node_shader_add_shared_sampler(kong, "texpaint_nor" + id + ": tex2d");
			node_shader_add_uniform(kong, "texpaint_size: float2", "_texpaint_size");
			node_shader_write_frag(kong, "var sample_matid: float = texpaint_nor" + id + "[int2(tex_coord * texpaint_size)].a + 1.0 / 255.0;");
			...
			node_shader_write_frag(kong, "output[1].rgba = float4(matid_r, matid_g, matid_b, 1.0);");
		}
			...
			node_shader_write_frag(kong, "output[1].rgba = float4(id_r, id_g, id_b, 1.0);");
		}
			...
			if (slot_layer_is_mask(context_raw.layer)) {
				let id: i32 = context_raw.layer.id;
				node_shader_write_frag(kong, "var mask_view: float = sample_lod_shared(texpaint" + id + ", tex_coord, 0.0).r;");
			}
			else {
				...
				for (let i: i32 = 0; i < slot_layer_get_masks(context_raw.layer).length; ++i) {
					...
					node_shader_write_frag(kong, "var mask_sample" + m.id + ": float = sample_lod_shared(texpaint_view_mask" + m.id + ", tex_coord, 0.0).r;");
					let opac: f32 = slot_layer_get_opacity(m);
			...
			node_shader_write_frag(kong, "output[1].rgba = float4(mask_view, mask_view, mask_view, 1.0);");
		}
		else {
	...
	node_shader_write_frag(kong, "output[2].rgba = float4(0.0, 0.0, tex_coord.xy);");
