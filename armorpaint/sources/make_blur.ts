...
function make_blur_run(kong: node_shader_t) {
	...
	if (context_raw.tool == workspace_tool_t.SMUDGE) {
		...
		node_shader_write_frag(kong, "var texpaint_pack_sample: float4 = sample(texpaint_pack_undo, sampler_linear, tex_coord_inp2) * blur_weight[i];");
		...
		node_shader_write_frag(kong, "nortan += sample(texpaint_nor_undo, sampler_linear, tex_coord_inp2).rgb * blur_weight[i];");
		...
		node_shader_write_frag(kong, "var texpaint_sample: float4 = sample(texpaint_undo, sampler_linear, tex_coord_inp + float2(blur_step * float(i), 0.0));");
		...
		node_shader_write_frag(kong, "var texpaint_pack_sample: float4 = sample(texpaint_pack_undo, sampler_linear, tex_coord_inp + float2(blur_step * float(i), 0.0)) * blur_weight[i + 7];");
		node_shader_write_frag(kong, "roughness += texpaint_pack_sample.g;");
		node_shader_write_frag(kong, "metallic += texpaint_pack_sample.b;");
		node_shader_write_frag(kong, "occlusion += texpaint_pack_sample.r;");
		node_shader_write_frag(kong, "height += texpaint_pack_sample.a;");
		node_shader_write_frag(kong, "nortan += sample(texpaint_nor_undo, sampler_linear, tex_coord_inp + float2(blur_step * float(i), 0.0)).rgb * blur_weight[i + 7];");
		node_shader_write_frag(kong, "}");
		// Y
		node_shader_write_frag(kong, "for (var j: int = -7; j <= 7; j += 1) {");
		node_shader_write_frag(kong, "var texpaint_sample: float4 = sample(texpaint_undo, sampler_linear, tex_coord_inp + float2(0.0, blur_step * float(j)));");
