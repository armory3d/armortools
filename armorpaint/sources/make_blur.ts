...
function make_blur_run(kong: node_shader_t) {
	node_shader_write_frag(kong, "var tex_coord_inp: float2 = gbuffer2[int2(sp.x * gbuffer_size.x, sp.y * gbuffer_size.y)].ba;");
	...
	node_shader_write_frag(kong, "var blur_step: float = 1.0 / texpaint_size.x;");
	if (context_raw.tool == workspace_tool_t.SMUDGE) {
		...
		node_shader_write_frag(kong, "var blur_direction: float2 = brush_direction.yx;");
		node_shader_write_frag(kong, "for (var i: int = 0; i < 7; i += 1) {");
		node_shader_write_frag(kong, "var tex_coord_inp2: float2 = gbuffer2[int2((sp.x + blur_direction.x * blur_step * float(i)) * gbuffer_size.x, (sp.y + blur_direction.y * blur_step * float(i)) * gbuffer_size.y)].ba;");
		node_shader_write_frag(kong, "var texpaint_sample: float4 = sample(texpaint_undo, tex_coord_inp2);");
		...
		node_shader_write_frag(kong, "var texpaint_pack_sample: float4 = sample(texpaint_pack_undo, tex_coord_inp2) * blur_weight[i];");
		...
		node_shader_write_frag(kong, "nortan += sample(texpaint_nor_undo, tex_coord_inp2).rgb * blur_weight[i];");
		...
		node_shader_write_frag(kong, "var texpaint_sample: float4 = sample(texpaint_undo, tex_coord_inp + float2(blur_step * float(i), 0.0));");
		...
		node_shader_write_frag(kong, "var texpaint_pack_sample: float4 = sample(texpaint_pack_undo, tex_coord_inp + float2(blur_step * float(i), 0.0)) * blur_weight[i + 7];");
		node_shader_write_frag(kong, "roughness += texpaint_pack_sample.g;");
		node_shader_write_frag(kong, "metallic += texpaint_pack_sample.b;");
		node_shader_write_frag(kong, "occlusion += texpaint_pack_sample.r;");
		node_shader_write_frag(kong, "height += texpaint_pack_sample.a;");
		node_shader_write_frag(kong, "nortan += sample(texpaint_nor_undo, tex_coord_inp + float2(blur_step * float(i), 0.0)).rgb * blur_weight[i + 7];");
		node_shader_write_frag(kong, "}");
		// Y
		node_shader_write_frag(kong, "for (var j: int = -7; j <= 7; j += 1) {");
		node_shader_write_frag(kong, "var texpaint_sample: float4 = sample(texpaint_undo, tex_coord_inp + float2(0.0, blur_step * float(j)));");
